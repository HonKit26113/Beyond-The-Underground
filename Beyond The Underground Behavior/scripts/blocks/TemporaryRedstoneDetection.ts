import { system, Block, world, ButtonPushAfterEvent, PressurePlatePopAfterEvent, LeverActionAfterEvent } from '@minecraft/server';
import { toggleOpenGate } from './FenceGates';
import { toggleOpenTrapdoor } from './Trapdoors';
import { toggleOpenDoor } from './Doors';

enum BlockTypes {
	FenceGate = "_fence_gate",
	Trapdoor = "_trapdoor",
	Door = "_door"
}

function handleAdjacentBlocks(
	block: Block,
	typeSubstring: string,
	toggleFn: (block: Block) => void,
	isButton = false
): void {
	const sides = [
		block.north(),
