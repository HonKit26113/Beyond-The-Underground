import { system, EquipmentSlot, BlockPermutation, BlockCustomComponent, GameMode, BlockTypes, Block, Player } from '@minecraft/server';
import { decrement_stack } from "../Functions";

// Function to calculate the new cardinal direction based on the player's rotation
function getNewCardinalDirection(currentDirection: string, angle: number) {
...
function directionDisplay(angle: number) {
...
export function toggleOpenGate(block: Block, player?: Player) {
...
const FenceGateComponent: BlockCustomComponent = {
    onPlayerInteract({block, player}, {}) {
		toggleOpenGate(block, player);
    },

	onRedstoneUpdate({block}, {}) {
		if (block.permutation.getState('kai:open' as any) === true) return; // if the gate is already open, powering it with redstone does nothing
