...
const doorBeforePlace: BlockCustomComponent = {
	beforeOnPlayerPlace({block, cancel}, {}) {
		const aboveBlock = block.above();
...
const doorOnPlace: BlockCustomComponent = {
	onPlace({block}, {}) {
		...
		if (aboveBlock.typeId === 'minecraft:air' && !block.permutation.getState('kai:upper_block_bit' as any)) {
...
const doorOpenComponent: BlockCustomComponent = {
	onPlayerInteract({block}, {}) {
		toggleOpenDoor(block);
	},
...
export function toggleOpenDoor(block: Block) {
	// Destructure event data for easier access
	const isUpperBlock = block.permutation.getState('kai:upper_block_bit' as any);
	...
	const currentOpenState = block.permutation.getState('kai:open_bit' as any);
	const targetOpenState = targetBlock.permutation.getState('kai:open_bit' as any);
...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent("honkit26113:door_interact", doorOpenComponent);
    blockComponentRegistry.registerCustomComponent("honkit26113:door_on_place", doorOnPlace);
    blockComponentRegistry.registerCustomComponent("honkit26113:door_before_place", doorBeforePlace);
});
