import { world, BlockPermutation, system, Player } from "@minecraft/server";
...
const useFertilizer = {
    onUseOn({ block, source }, {}) {
        ...
        if (!smallTreeIds.includes(id) && !largeTreeIds.includes(id) && !cropIds.includes(id) && !specialIds.has(id))
            return;
        // Short grass
        ...
        if (smallTreeIds.includes(id)) {
            ...
            switch (id) {
                ...
                case "minecraft:spruce_sapling":
                    if (checkLargeTree(block, source))
                        break;
...
function checkLargeTree(block, source) {
    const id = block.typeId;
    const dim = block.dimension;
 ...
 */
function checkSpace(block, structure) {
...
function sendErrorMessage(player) {
...
//# sourceMappingURL=GypsumFertilizer.js.map